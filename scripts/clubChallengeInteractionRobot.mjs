...
check('player: public voting has a submit action', contains(publicVote,'Submit My Votes'));
check('voting: one ballot per phone/browser is enforced with a hashed device key', contains(voteFn,'deviceKey(event.id, rawDeviceId)') && contains(voteFn,'voter_identity_key:voterIdentityKey') && contains(voteFn,'already voted'));
check('voting: replaced rotation players remain eligible while unused reserves are excluded', contains(voteFn,'isUnusedReserve') && contains(voteFn,'reserve_activated') && !contains(voteFn,"nominee.status === 'replaced'"));
check('voting: ballot audit omits nominee identities', /nominee identities intentionally omitted/i.test(voteFn) && contains(voteFn,"action:'pot_ballot_cast'"));
