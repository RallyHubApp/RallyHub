    ...
    <div className="grid lg:grid-cols-[minmax(0,1fr)_320px] gap-4 items-start">
      <div className="space-y-4 min-w-0">
        ...
        <section className="glass rounded-2xl p-4 sm:p-5" data-testid="kotc-setup-round1">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div><Label>Starting order</Label><Select value={seedingSource} onValueChange={applySeedingSource}><SelectTrigger className="mt-1 min-h-11" data-testid="kotc-seeding-source"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="roster">Roster order</SelectItem><SelectItem value="manual">Manual ranking</SelectItem>{!sandboxMode&&<><SelectItem value="dupr">Genuine DUPR</SelectItem><SelectItem value="previous_kotc">Previous KOTC performance</SelectItem></>}</SelectContent></Select><p className="text-[11px] text-muted-foreground mt-1.5">{sandboxMode?'Dummy guests use roster order or a manual test order only.':'This sets the starting order only. RallyHub does not invent ratings.'}</p></div>
            <div><Label>Round 1 draw</Label><Select value={drawMethod} onValueChange={setDrawMethod}><SelectTrigger className="mt-1 min-h-11" data-testid="kotc-draw-method"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="balanced">Balanced Ranking</SelectItem><SelectItem value="pure_random">Pure Random</SelectItem><SelectItem value="strict">Strict Ranking</SelectItem></SelectContent></Select><p className="text-[11px] text-muted-foreground mt-1.5">Balanced Ranking uses your 1-to-N order to spread strength across the courts and, where possible, pairs the strongest player with the weakest in that court. Strict Ranking keeps the strongest four together, then the next four, and so on.</p></div>
          </div>
          {!sandboxMode&&seedingSource==='dupr'&&players.some(p=>p.dupr_rating==null)&&<div className="rounded-lg border border-amber-400/30 bg-amber-500/10 p-2.5 text-xs text-amber-600 mt-3">{players.filter(p=>p.dupr_rating==null).length} player{players.filter(p=>p.dupr_rating==null).length===1?' has':'s have'} no genuine DUPR rating. They stay below rated players rather than receiving a made-up value.</div>}
