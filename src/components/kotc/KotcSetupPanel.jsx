    ...
    <div className="grid lg:grid-cols-[minmax(0,1fr)_320px] gap-4 items-start">
      <div className="space-y-4 min-w-0">
        ...
        <section className="glass rounded-2xl p-4 sm:p-5" data-testid="kotc-setup-round1">
          ...
          {rankingOpen&&<div className="mt-3 rounded-xl border bg-secondary/15 overflow-hidden"><div className="px-3 py-2 border-b bg-secondary/30 flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-semibold">Current starting order</p><p className="text-[10px] text-muted-foreground">Moving anyone manually changes the source to Manual ranking.</p></div><Badge variant="outline">{seedingLabel[seedingSource]||seedingSource}</Badge></div><div className="max-h-[420px] overflow-y-auto divide-y">{orderedPlayers.map((p,i)=><div key={p.id} className="flex items-center gap-2 px-3 py-2.5"><span className="w-7 text-xs font-bold text-muted-foreground">#{i+1}</span><span className="flex-1 text-sm font-medium min-w-0 truncate">{p.full_name}</span>{p.dupr_rating!=null&&<Badge variant="outline" className="hidden sm:inline-flex">DUPR {Number(p.dupr_rating).toFixed(2)}</Badge>}<div className="flex gap-1"><Button variant="ghost" size="sm" aria-label={`Move ${p.full_name} up`} disabled={i===0} onClick={()=>{setSeedingSource('manual');setPlayerOrder(o=>{const n=[...o];[n[i-1],n[i]]=[n[i],n[i-1]];return n;});}}>↑</Button><Button variant="ghost" size="sm" aria-label={`Move ${p.full_name} down`} disabled={i===orderedPlayers.length-1} onClick={()=>{setSeedingSource('manual');setPlayerOrder(o=>{const n=[...o];[n[i],n[i+1]]=[n[i+1],n[i]];return n;});}}>↓</Button></div></div>)}</div></div>}
        </section>
