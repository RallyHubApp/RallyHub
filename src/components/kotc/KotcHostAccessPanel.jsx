  ...
  return <div className="glass rounded-xl p-3 sm:p-4 space-y-3 border border-primary/20">
    ...
    {open&&<div className="pt-2 border-t border-border space-y-3">
      <div className="grid sm:grid-cols-2 gap-2">
        ...
        <div className="rounded-lg bg-secondary/40 p-3 space-y-2"><p className="text-xs font-semibold flex items-center gap-2"><Link2 className="w-3.5 h-3.5"/>Scorer Link</p><p className="text-[10px] text-muted-foreground">Can score/correct the current round only. Per-court edit locking prevents two scorers overwriting each other.</p><Button size="sm" variant="outline" onClick={prepareScorer} disabled={loading}>{copied==='scorer'?<Check className="w-3 h-3 mr-1"/>:<Copy className="w-3 h-3 mr-1"/>}{copied==='scorer'?'Copied':'Copy Scorer Link'}</Button>{scorerUrl&&<p className="text-[9px] break-all text-muted-foreground">{scorerUrl}</p>}</div>
      </div>
