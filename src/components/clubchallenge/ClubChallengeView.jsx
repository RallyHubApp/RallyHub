      ...
      {tab === 'draw' && (
        <div className="space-y-4">
          {!matches.length ? <div className="glass rounded-xl p-8 text-center text-sm text-muted-foreground">Generate a draw from Teams & Ranking first.</div> : <>
            ...
            {event?.status === 'draw_approved' && <div className="rounded-xl border border-primary/25 bg-primary/5 p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p className="text-xs font-bold">Hall sound check before play</p><p className="text-[10px] text-muted-foreground">Use the actual phone/tablet and speaker now. This is local audio only and makes no Base44 request.</p></div><Button variant="outline" className="min-h-11" onClick={() => unlockHallAudio({test:true})}>{audioReady ? 'Test Sound Again ✓' : 'Test Sound'}</Button></div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{event?.status === 'draw_generated' && <><Button variant="outline" className="w-full min-h-11" onClick={generateDraw} disabled={saving || !!hostAction}><RefreshCw className="w-4 h-4 mr-2" />Full Redraw</Button><Button className="w-full min-h-11" onClick={approveDraw} disabled={!!hostAction}><Check className="w-4 h-4 mr-2" />Approve Draw</Button></>}{event?.status === 'draw_approved' && <Button className="w-full" onClick={startEvent} disabled={!!hostAction}><Play className="w-4 h-4 mr-2" />Start Club Challenge</Button>}</div>
          </>}
        </div>
      )}
