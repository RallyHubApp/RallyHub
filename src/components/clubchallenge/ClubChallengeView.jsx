        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed','archived'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <details id="cc-court-time-controls" className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="border-t border-border p-4 sm:p-5"><div className="rounded-lg bg-secondary/30 p-4 space-y-3"><div><p className="text-sm font-semibold">Preview the impact before changing anything</p><p className="text-xs text-muted-foreground">Enter the courts actually available now and the minutes remaining. RallyHub will show how many future matches still fit, which matches would move, and whether any would have to be marked Not Played. Completed results are never changed.</p></div><div className="grid sm:grid-cols-[140px_180px_auto] gap-2 items-end"><div><Label className="text-xs">Courts available now</Label><Input type="number" min="1" value={eventDayAdjust.courts} onChange={e=>setEventDayAdjust(x=>({...x,courts:e.target.value}))} placeholder={`${event.courts}`} className="mt-1 bg-secondary" /></div><div><Label className="text-xs">Minutes remaining</Label><Input type="number" min="1" value={eventDayAdjust.availableMinutes} onChange={e=>setEventDayAdjust(x=>({...x,availableMinutes:e.target.value}))} placeholder="e.g. 60" className="mt-1 bg-secondary" /></div><Button variant="outline" disabled={!canManageEvent} onClick={proposeEventDayAdjustment}>Preview Impact</Button></div>{eventDayProposal && <div className="rounded-lg bg-secondary/50 p-3 text-xs space-y-2"><p><strong>Proposed change:</strong> {eventDayProposal.unresolved - eventDayProposal.dropIds.length} future matches can still be played · {eventDayProposal.changes.length} move to a different round/court · {eventDayProposal.dropIds.length} would be marked Not Played.</p><p className="text-muted-foreground">Nothing changes until you press Confirm Changes. Completed results remain locked, and the existing Event Pack will be marked out of date.</p><div className="flex gap-2"><Button size="sm" disabled={!canManageEvent} onClick={confirmEventDayAdjustment}>Confirm Changes</Button><Button size="sm" variant="outline" onClick={()=>setEventDayProposal(null)}>Cancel</Button></div></div>}</div></div>
            </details>
          </>}
        </div>
      )}
