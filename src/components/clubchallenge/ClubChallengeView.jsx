          ...
          {!event || !['in_progress','paused','completed'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and Start Club Challenge first.</div> : <>
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-3">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-2 rounded-lg bg-secondary/40 p-3">
                ...
                <div><Label className="text-xs">Hall volume · {Math.round(hallVolume * 100)}%</Label><input aria-label="Club Challenge hall volume" type="range" min="0" max="1" step="0.05" value={hallVolume} onChange={e => setHallVolume(Number(e.target.value))} className="mt-3 w-full" /></div>
