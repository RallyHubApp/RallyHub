...
export default function ClubChallengeView({ tournament, queryClient, isAdmin }) {
  ...
  if ((displayMode || displayOnly) && event) return (
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-4">
      ...
      {event.pot_enabled && event.pot_status === 'open' && <div className="rounded-2xl border-2 border-primary/40 bg-primary/10 p-4 text-center"><p className="text-sm sm:text-lg font-black uppercase tracking-[.16em] text-primary">Players of the Tournament voting open</p><p className="mt-1 text-2xl sm:text-4xl font-black tabular-nums">{potCountdownText}</p><p className="mt-1 text-xs sm:text-sm text-muted-foreground">Scan the event voting QR and choose one player from each team.</p></div>}
