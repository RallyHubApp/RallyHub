...
export default function ClubChallengeView({ tournament, queryClient, isAdmin }) {
  ...
  if ((displayMode || displayOnly) && event) return (
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-4">
      ...
      <div className="flex justify-between items-start gap-4"><div className="flex-1 text-center"><p className="text-sm uppercase tracking-[.2em] text-primary font-bold">{INTERCLUB_MODULE_NAME} · Hall Display</p><div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-8"><div className="flex items-center justify-end gap-3 min-w-0">{event.club_a_logo_url&&<img src={event.club_a_logo_url} alt="" className="w-12 h-12 sm:w-20 sm:h-20 rounded-xl bg-white object-contain p-1"/>}<span className="text-lg sm:text-4xl font-black truncate">{event.club_a_name}</span></div><span className="text-4xl sm:text-7xl font-black text-primary tabular-nums whitespace-nowrap">{score.clubA} – {score.clubB}</span><div className="flex items-center justify-start gap-3 min-w-0"><span className="text-lg sm:text-4xl font-black truncate">{event.club_b_name}</span>{event.club_b_logo_url&&<img src={event.club_b_logo_url} alt="" className="w-12 h-12 sm:w-20 sm:h-20 rounded-xl bg-white object-contain p-1"/>}</div></div></div>{!displayOnly && <Button variant="outline" onClick={() => setDisplayMode(false)}>Exit Display</Button>}</div>
  ...
  return (
    <div data-testid="cc-root" className="space-y-4 print:space-y-0">
      ...
      {publicLinks && <div className="print:hidden rounded-xl border border-primary/20 bg-card p-4 space-y-4"><div><p className="text-sm font-semibold">Public {INTERCLUB_MODULE_NAME} Links</p><p className="text-xs text-muted-foreground">Use the Hall Display link on a TV/tablet. Anyone at the event can scan the Players of the Tournament QR and submit one ballot from their phone/browser.</p></div><div className="grid md:grid-cols-2 gap-4"><div className="rounded-lg bg-secondary/40 p-4 flex gap-4 items-center"><QRCodeSVG value={publicLinks.displayUrl} size={104}/><div className="min-w-0"><p className="text-xs font-semibold">Hall Display</p><p className="text-[10px] text-muted-foreground break-all mt-1">{publicLinks.displayUrl}</p><Button size="sm" variant="outline" className="mt-2" onClick={() => navigator.clipboard?.writeText(publicLinks.displayUrl)}>Copy link</Button></div></div><div className="rounded-lg bg-secondary/40 p-4 flex gap-4 items-center"><QRCodeSVG value={publicLinks.votingUrl} size={104}/><div className="min-w-0"><p className="text-xs font-semibold">Players of the Tournament Voting</p><p className="text-[10px] text-muted-foreground break-all mt-1">{publicLinks.votingUrl}</p><Button size="sm" variant="outline" className="mt-2" onClick={() => navigator.clipboard?.writeText(publicLinks.votingUrl)}>Copy voting link</Button></div></div></div></div>}
