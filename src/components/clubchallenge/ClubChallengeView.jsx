            ...
            <details id="cc-player-controls" className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="border-t border-border p-4 sm:p-5 space-y-4">
                <div className="rounded-lg bg-secondary/30 p-4 space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2"><Button data-testid="cc-replace-player" className="w-full sm:w-auto" disabled={!canManageEvent || playerControlBusy || !replacement.outgoingId || !replacement.incomingName.trim()} onClick={applyReplacement}>{playerControlBusy ? 'Applying…' : `Replace from Round ${currentRound}`}</Button><Button variant="outline" className="w-full sm:w-auto" disabled={!canManageEvent || playerControlBusy || !replacement.outgoingId} onClick={withdrawWithoutReplacement}>Continue Short · No Replacement</Button></div>
