              ...
              <div className="border-t border-border p-4 sm:p-5 space-y-4">
                <div className="rounded-lg bg-secondary/30 p-4 space-y-3">
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2">
                    <div><Label className="text-xs">Player leaving</Label><Select value={replacement.outgoingId} onValueChange={v => { setPlayerControlStatus(null); setReplacement(r => ({ ...r, outgoingId:v, candidateId:'', incomingName:'', incomingGender:'', incomingSourcePlayerId:'', incomingParticipantType:'' })); }} disabled={playerControlBusy}><SelectTrigger data-testid="cc-replacement-outgoing" className="mt-1 bg-secondary"><SelectValue placeholder="Choose player" /></SelectTrigger><SelectContent>{participants.filter(p => p.status === 'active').map(p => <SelectItem key={p.id} value={p.id}>{p.display_name} · {p.side === 'club_a' ? event.club_a_name : event.club_b_name}</SelectItem>)}</SelectContent></Select></div>
                    ...
                    <div><Label className="text-xs">Replacement name</Label><Input data-testid="cc-replacement-name" value={replacement.incomingName} onChange={e => { setPlayerControlStatus(null); setReplacement(r => ({ ...r, candidateId:'', incomingName:e.target.value, incomingSourcePlayerId:'', incomingParticipantType:'' })); }} placeholder="Name" className="mt-1 bg-secondary" disabled={playerControlBusy} /></div>
