      ...
      {tab === 'draw' && (
        <div className="space-y-4">
          {!matches.length ? <div className="glass rounded-xl p-8 text-center text-sm text-muted-foreground">Generate a draw from Teams & Ranking first.</div> : <>
            ...
            {event?.status === 'draw_approved' && <div className="rounded-xl border border-primary/25 bg-primary/5 p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p className="text-xs font-bold">Hall sound & PA check before play</p><p className="text-[10px] text-muted-foreground">Set the laptop audio output to the JBL Charge 6. Test the RallyHub cue, then test the laptop microphone as a live PA before starting the event.</p>{paMicLabel && <p className="text-[10px] text-muted-foreground mt-1">Mic: {paMicLabel}</p>}{paError && <p className="text-[10px] text-destructive mt-1">{paError}</p>}</div><div className="flex flex-col sm:flex-row gap-2"><Button data-testid="cc-prestart-sound-check" variant="outline" className="min-h-11" onClick={() => unlockHallAudio({test:true})}>{audioReady ? 'Test Sound Again ✓' : 'Test Sound'}</Button><Button data-testid="cc-prestart-pa-test" className="min-h-11" variant={paActive ? 'destructive' : 'default'} disabled={paStarting} onClick={paActive ? stopPA : startPA}>{paActive ? <><MicOff className="w-4 h-4 mr-2" />Stop PA</> : <><Mic className="w-4 h-4 mr-2" />{paStarting ? 'Starting…' : 'Test PA Mic'}</>}</Button></div></div>}
        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-4', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              ...
              {paMicLabel && <p className="text-[11px] text-muted-foreground"><strong>Microphone:</strong> {paMicLabel}</p>}
