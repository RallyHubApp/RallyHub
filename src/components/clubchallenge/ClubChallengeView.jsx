...
export default function ClubChallengeView({ tournament, queryClient, isAdmin }) {
  ...
  if ((displayMode || displayOnly) && event) return (
    <div className="min-h-[75vh] bg-background p-4 sm:p-8 space-y-6">
      ...
      {showcaseDisplayActive ? <div className="rounded-2xl border-2 border-primary/40 bg-card p-6 sm:p-8 text-center shadow-lg"><div className="flex flex-wrap justify-center gap-2"><Badge>{showcaseMatch.showcase_mode === 'exhibition' ? 'OPTIONAL SHOWCASE · EXHIBITION' : 'SHOWCASE TIEBREAK'}</Badge><Badge variant="outline">First to {showcaseMatch.showcase_target_points || 11} · win by {showcaseMatch.showcase_win_by || 1}</Badge></div><p className="mt-5 text-lg sm:text-2xl font-bold">{(showcaseMatch.club_a_names || []).map(n => privacyName(n, !!event.junior_display_mode)).join(' & ')}</p><p className="my-2 text-xs uppercase tracking-widest text-muted-foreground">vs</p><p className="text-lg sm:text-2xl font-bold">{(showcaseMatch.club_b_names || []).map(n => privacyName(n, !!event.junior_display_mode)).join(' & ')}</p><p className="mt-6 text-7xl sm:text-9xl font-black tabular-nums text-primary">{showcaseMatch.score_a ?? 0} – {showcaseMatch.score_b ?? 0}</p>{showcaseSideChangeRecent && <div className="mt-6 rounded-xl bg-red-600 px-4 py-4 text-2xl sm:text-4xl font-black text-white">CHANGE ENDS</div>}{showcaseMatch.status === 'completed' && <p className="mt-5 text-xl font-bold">{showcaseMatch.winner === 'club_a' ? event.club_a_name : event.club_b_name} won the Showcase {showcaseMatch.score_a}–{showcaseMatch.score_b}</p>}{showcaseMatch.showcase_mode === 'exhibition' && <p className="mt-3 text-sm text-muted-foreground">Exhibition only · the Interclub result above is unchanged</p>}</div> : <div className={cn('rounded-2xl border p-6 text-center', breakActive ? 'border-red-400 bg-red-600 text-white shadow-xl' : 'border-border bg-card')}><p className={cn('text-lg uppercase tracking-widest', breakActive ? 'font-black text-white' : 'text-muted-foreground')}>{breakActive ? 'BREAK NOW' : <>{roundLabel(currentRound)} · {timerState?.phase || 'idle'}</>}</p><p className="text-7xl sm:text-9xl font-bold tabular-nums mt-2">{fmtTimer(timerRemaining)}</p>{breakActive && <p className="mt-3 text-sm sm:text-lg text-white/90">Enjoy the break · Round {currentRound + 1} is up next</p>}</div>}
