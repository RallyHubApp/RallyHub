...
export default function ClubChallengeView({ tournament, queryClient, isAdmin }) {
  ...
  if ((displayMode || displayOnly) && event && displayCompleted && !showcaseDisplayActive) return (
    <div className="min-h-screen bg-background p-5 sm:p-10 flex flex-col justify-center">
      <div className="mx-auto w-full max-w-6xl text-center">
        ...
        {event.pot_status==='revealed'&&potWinnerIds.length>0&&<div className="mt-8 rounded-3xl border-2 border-primary/30 bg-primary/5 p-5 sm:p-8"><p className="text-sm sm:text-lg uppercase tracking-[.2em] text-primary font-black">Players of the Tournament</p><div className="mt-5 grid sm:grid-cols-2 gap-4"><div className="rounded-2xl bg-card border p-5"><p className="text-sm text-muted-foreground">{event.club_a_name}</p><p className="mt-2 text-xl sm:text-3xl font-black">{potWinnersA.map(p=>privacyName(p.display_name, !!event.junior_display_mode)).join(' & ')}</p></div><div className="rounded-2xl bg-card border p-5"><p className="text-sm text-muted-foreground">{event.club_b_name}</p><p className="mt-2 text-xl sm:text-3xl font-black">{potWinnersB.map(p=>privacyName(p.display_name, !!event.junior_display_mode)).join(' & ')}</p></div></div></div>}
  ...
  if ((displayMode || displayOnly) && event) return (
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-4">
      ...
      {showcaseDisplayActive ? <div className="flex min-h-[calc(100vh-10rem)] flex-col">
        ...
        <div className="mt-4 flex-1 grid grid-cols-[1fr_auto_1fr] items-stretch gap-3 sm:gap-8">
  ...
  return (
    <div data-testid="cc-root" className="space-y-4 print:space-y-0">
      ...
      {publicLinks && <div className="print:hidden rounded-xl border border-primary/20 bg-card p-4 space-y-4"><div><p className="text-sm font-semibold">Public {INTERCLUB_MODULE_NAME} Links</p><p className="text-xs text-muted-foreground">Share the Live Event View with players for phones or use it on a larger screen. When voting opens, players can tap the voting link directly or scan the QR code.</p></div><div className="grid md:grid-cols-2 gap-4"><div className="rounded-lg bg-secondary/40 p-4 flex flex-col sm:flex-row gap-4 items-center"><QRCodeSVG value={publicLinks.displayUrl} size={132} level="H" includeMargin/><div className="min-w-0 flex-1"><p className="text-xs font-semibold">Live Event View</p><a href={publicLinks.displayUrl} target="_blank" rel="noreferrer" className="text-[10px] text-primary underline underline-offset-2 break-all mt-1 block">{publicLinks.displayUrl}</a><div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => navigator.clipboard?.writeText(publicLinks.displayUrl)}>Copy</Button><Button size="sm" variant="outline" onClick={() => sharePublicLink(publicLinks.displayUrl,'Live Event View')}>Share</Button><Button size="sm" variant="outline" onClick={() => shareOnWhatsApp(publicLinks.displayUrl,'Live Event View')}>WhatsApp</Button></div></div></div><div className="rounded-lg bg-secondary/40 p-4 flex flex-col sm:flex-row gap-4 items-center"><QRCodeSVG value={publicLinks.votingUrl} size={132} level="H" includeMargin/><div className="min-w-0 flex-1"><p className="text-xs font-semibold">Players of the Tournament Voting</p><a href={publicLinks.votingUrl} target="_blank" rel="noreferrer" className="text-[10px] text-primary underline underline-offset-2 break-all mt-1 block">{publicLinks.votingUrl}</a><div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => navigator.clipboard?.writeText(publicLinks.votingUrl)}>Copy</Button><Button size="sm" variant="outline" onClick={() => sharePublicLink(publicLinks.votingUrl,'Players of the Tournament Voting')}>Share</Button><Button size="sm" variant="outline" onClick={() => shareOnWhatsApp(publicLinks.votingUrl,'Players of the Tournament Voting')}>WhatsApp</Button></div></div></div></div></div>}
            ...
            <div ref={hostBarAnchorRef} style={hostBarPinned ? { height: hostBarGeometry.height } : undefined}>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <div className="ml-auto flex flex-wrap items-center gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => { const panel = document.getElementById('cc-player-controls'); if (panel instanceof HTMLDetailsElement) { panel.open = true; panel.scrollIntoView({ behavior:'smooth', block:'center' }); } }}><Users className="w-4 h-4 mr-1" />Player Changes</Button>
                  {!['completed','archived'].includes(event.status) && (breakActive ? <Button size="sm" variant="destructive" disabled={!canManageEvent} onClick={timerRemaining > 0 ? endBreakEarly : advanceRound}>{timerRemaining > 0 ? `End Break → R${currentRound + 1}` : `Prepare R${currentRound + 1}`}</Button> : <>
                    ...
                    <Button size="sm" disabled={advanceActionDisabled} onClick={advanceRound}>{advanceActionLabel}</Button>
                  </>)}
                </div>
              </div>
