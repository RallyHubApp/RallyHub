          ...
          {!event || !['in_progress','paused','completed','archived'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-4', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
                <Button data-testid="cc-pa-toggle" className="h-12" variant={paActive ? 'destructive' : 'default'} disabled={!hasManagePermission || paStarting} onClick={paActive ? stopPA : startPA}>{paActive ? <><MicOff className="w-4 h-4 mr-2" />Stop PA</> : <><Mic className="w-4 h-4 mr-2" />{paStarting ? 'Starting PA…' : 'Start PA'}</>}</Button>
