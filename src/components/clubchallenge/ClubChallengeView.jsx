        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and Start Club Challenge first.</div> : <>
            ...
            <div className="grid lg:grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border border-border bg-card p-4 space-y-3"><div><p className="text-sm font-semibold">Court / Time Change Proposal</p><p className="text-xs text-muted-foreground">For a lost/added court or late running event. This calculates consequences first; it does not silently rewrite completed fixtures.</p></div><div className="grid sm:grid-cols-[120px_140px_auto] gap-2"><Input type="number" min="1" value={eventDayAdjust.courts} onChange={e=>setEventDayAdjust(x=>({...x,courts:e.target.value}))} placeholder={`${event.courts} courts`} className="bg-secondary" /><Input type="number" min="1" value={eventDayAdjust.availableMinutes} onChange={e=>setEventDayAdjust(x=>({...x,availableMinutes:e.target.value}))} placeholder="Minutes left" className="bg-secondary" /><Button variant="outline" onClick={proposeEventDayAdjustment}>Calculate Proposal</Button></div></div>
            </div>
          ...
          {score.completedMatches > 0 ? (
            <>
              ...
              {event?.pot_enabled && <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                ...
                {isAdmin && <div className="flex flex-wrap gap-2">{event.pot_status !== 'open' && event.pot_status !== 'revealed' && <Button variant="outline" onClick={() => setPotStatus('open')}>Open Voting</Button>}{event.pot_status === 'open' && <Button variant="outline" onClick={() => setPotStatus('closed')}>Close Voting</Button>}{event.pot_status === 'closed' && <Button onClick={revealPot} disabled={!potVotes.length}>Reveal Result</Button>}</div>}
                {event.pot_status === 'open' && <div className="grid md:grid-cols-[1fr_1fr_auto] gap-2 items-end"><div><Label className="text-xs">Voting player</Label><Select value={potVoterId} onValueChange={setPotVoterId}><SelectTrigger className="mt-1 bg-secondary"><SelectValue placeholder="Select your name" /></SelectTrigger><SelectContent>{participants.filter(p => p.status === 'active').map(p => <SelectItem key={p.id} value={p.id}>{p.display_name}</SelectItem>)}</SelectContent></Select></div><div><Label className="text-xs">Your Player of Tournament</Label><Select value={potNomineeId} onValueChange={setPotNomineeId}><SelectTrigger className="mt-1 bg-secondary"><SelectValue placeholder="Select player" /></SelectTrigger><SelectContent>{participants.filter(p => p.status === 'active' && p.id !== potVoterId).map(p => <SelectItem key={p.id} value={p.id}>{p.display_name}</SelectItem>)}</SelectContent></Select></div><Button onClick={castPotVote} disabled={!potVoterId || !potNomineeId}>Cast Vote</Button></div>}
                {event.pot_status !== 'revealed' && <p className="text-xs text-muted-foreground">{potVotes.length} vote{potVotes.length === 1 ? '' : 's'} securely recorded. Individual totals are hidden.</p>}
                {event.pot_status === 'revealed' && <div className="rounded-lg bg-primary/10 p-4 text-center"><Trophy className="w-5 h-5 text-primary mx-auto" /><p className="font-bold mt-2">{potWinnerNames.length > 1 ? 'Joint Players of the Tournament' : 'Player of the Tournament'}</p><p className="text-lg mt-1">{potWinnerNames.join(' & ') || 'No valid votes'}</p>{potWinnerNames.map(name => { const p = participants.find(x => x.display_name === name); return <p key={name} className="text-xs text-muted-foreground">{name}: {potCounts[p?.id] || 0} votes</p>; })}</div>}
              </div>}
