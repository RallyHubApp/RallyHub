        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-4', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              ...
              <div className="grid sm:grid-cols-[180px_1fr_auto] gap-2 items-end"><div><Label className="text-xs">Voice announcements</Label><Select value={voiceMode} onValueChange={setVoiceMode}><SelectTrigger className="mt-1 bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="device_default">On · Device default</SelectItem><SelectItem value="off">Off</SelectItem></SelectContent></Select></div><Input value={announcementDraft} onChange={e => { setAnnouncementDraft(e.target.value); if (!announcementSpeaking) setAnnouncementStatus(''); }} onKeyDown={e => { if (e.key === 'Enter' && !announcementSpeaking) announceCustom(); }} placeholder="Type a one-off RallyHub announcement…" className="bg-secondary" disabled={announcementSpeaking} /><Button variant="outline" disabled={!announcementDraft.trim() || paActive || announcementSpeaking || voiceMode === 'off'} onClick={announceCustom}><Megaphone className="w-4 h-4 mr-2" />{announcementSpeaking ? 'Speaking…' : 'Announce'}</Button></div>{announcementStatus && <p className="text-[11px] text-muted-foreground">{announcementStatus}</p>}
