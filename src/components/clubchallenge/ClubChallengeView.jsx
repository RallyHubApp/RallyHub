          ...
          {!event || !['in_progress','paused','completed','archived'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            <div className="sticky top-2 z-30 rounded-xl border border-primary/30 bg-background/95 backdrop-blur px-3 py-2 shadow-lg">
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <div className="ml-auto flex flex-wrap items-center gap-2">
                  <Button size="sm" variant="outline" onClick={() => document.getElementById('cc-pa-panel')?.scrollIntoView({ behavior:'smooth', block:'center' })}><Mic className="w-4 h-4 mr-1" />PA</Button>
            ...
            <div className="rounded-xl border border-primary/25 bg-card p-4 sm:p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                ...
                {event.include_break && currentRound === event.break_after_round ? <Button variant="outline" className="min-h-12" disabled={!canManageEvent || !changeoverAvailable} onClick={() => startPhase('break')}>Start {event.break_minutes}-min Break</Button> : <Button variant="outline" className="min-h-12" disabled={!canManageEvent || !timerRunning || timerPhase !== 'play'} onClick={() => timerAction('add_minute')}>+1 minute</Button>}
            ...
            <details id="cc-player-controls" className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="border-t border-border p-4 sm:p-5 space-y-4">
                <div className="rounded-lg bg-secondary/30 p-4 space-y-3">
                  <div><p className="text-sm font-semibold">Replace / Withdraw a Player</p><p className="text-xs text-muted-foreground">Completed results stay unchanged. RallyHub updates future unplayed fixtures only.</p></div>
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2">
                    <div><Label className="text-xs">Player leaving</Label><Select value={replacement.outgoingId} onValueChange={v => setReplacement(r => ({ ...r, outgoingId:v, candidateId:'', incomingName:'', incomingGender:'', incomingSourcePlayerId:'', incomingParticipantType:'' }))}><SelectTrigger className="mt-1 bg-secondary"><SelectValue placeholder="Choose player" /></SelectTrigger><SelectContent>{participants.filter(p => p.status === 'active').map(p => <SelectItem key={p.id} value={p.id}>{p.display_name} · {p.side === 'club_a' ? event.club_a_name : event.club_b_name}</SelectItem>)}</SelectContent></Select></div>
                    <div><Label className="text-xs">Registered reserve / available player</Label><Select value={replacement.candidateId || 'manual'} onValueChange={v => { if (v === 'manual') { setReplacement(r => ({ ...r, candidateId:'', incomingName:'', incomingGender:'', incomingSourcePlayerId:'', incomingParticipantType:'' })); return; } const c = availableReplacementCandidates.find(x => x.id === v); if (c) setReplacement(r => ({ ...r, candidateId:c.id, incomingName:c.displayName || '', incomingGender:c.gender || '', incomingSourcePlayerId:c.sourcePlayerId || '', incomingParticipantType:c.participantType || '' })); }} disabled={!replacement.outgoingId}><SelectTrigger className="mt-1 bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="manual">Type a replacement manually</SelectItem>{availableReplacementCandidates.map(c => <SelectItem key={c.id} value={c.id}>{c.displayName}</SelectItem>)}</SelectContent></Select></div>
                    <div><Label className="text-xs">Replacement name</Label><Input value={replacement.incomingName} onChange={e => setReplacement(r => ({ ...r, candidateId:'', incomingName:e.target.value, incomingSourcePlayerId:'', incomingParticipantType:'' }))} placeholder="Name" className="mt-1 bg-secondary" /></div>
                  ...
                  {replacement.outgoingId && availableReplacementCandidates.length === 0 && <p className="text-[11px] text-muted-foreground">No unused registered player from this club is currently available in the event list. You can still type the replacement name manually.</p>}
                  <div className="flex flex-col sm:flex-row gap-2"><Button className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId || !replacement.incomingName.trim()} onClick={applyReplacement}>Replace from Round {currentRound}</Button><Button variant="outline" className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId} onClick={withdrawWithoutReplacement}>Continue Short · No Replacement</Button></div>
                </div>
            ...
            <details id="cc-court-time-controls" className="rounded-xl border border-border bg-card overflow-hidden">
              <summary className="cursor-pointer list-none p-4 sm:p-5 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">Court & Time Changes</p><p className="text-xs text-muted-foreground">Use only if a court is lost/added or the event is running late.</p></div><ChevronDown className="w-4 h-4 text-muted-foreground" /></summary>
              <div className="border-t border-border p-4 sm:p-5"><div className="rounded-lg bg-secondary/30 p-4 space-y-3"><div><p className="text-sm font-semibold">Review a change before applying it</p><p className="text-xs text-muted-foreground">RallyHub calculates the effect first and never silently rewrites completed results.</p></div><div className="grid sm:grid-cols-[120px_160px_auto] gap-2"><Input type="number" min="1" value={eventDayAdjust.courts} onChange={e=>setEventDayAdjust(x=>({...x,courts:e.target.value}))} placeholder={`${event.courts} courts`} className="bg-secondary" /><Input type="number" min="1" value={eventDayAdjust.availableMinutes} onChange={e=>setEventDayAdjust(x=>({...x,availableMinutes:e.target.value}))} placeholder="Minutes remaining" className="bg-secondary" /><Button variant="outline" disabled={!canManageEvent} onClick={proposeEventDayAdjustment}>Calculate Impact</Button></div>{eventDayProposal && <div className="rounded-lg bg-secondary/50 p-3 text-xs space-y-2"><p><strong>Review:</strong> {eventDayProposal.unresolved - eventDayProposal.dropIds.length} matches retained · {eventDayProposal.changes.length} positions change · {eventDayProposal.dropIds.length} Not Played.</p><p className="text-muted-foreground">Completed results are locked. Event Pack becomes OUT OF DATE.</p><div className="flex gap-2"><Button size="sm" disabled={!canManageEvent} onClick={confirmEventDayAdjustment}>Confirm Changes</Button><Button size="sm" variant="outline" onClick={()=>setEventDayProposal(null)}>Cancel</Button></div></div>}</div></div>
            </details>
          </>}
        </div>
      )}
