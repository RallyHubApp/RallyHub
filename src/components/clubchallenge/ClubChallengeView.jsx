        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed','archived'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-4', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              ...
              <div className="space-y-2"><p className="text-xs font-semibold">Voice announcements</p><div className="flex flex-col md:flex-row gap-2"><Button className="w-full md:w-36 md:shrink-0" variant="outline" disabled={!announcementDraft.trim() || paActive || announcementSpeaking} onClick={announceCustom}><Megaphone className="w-4 h-4 mr-2" />{announcementSpeaking ? 'Speaking…' : 'Announce'}</Button><div className="w-full md:max-w-4xl"><Label className="sr-only">Announcement text</Label><Input type="text" name="rallyhub-announcement-text" autoComplete="off" inputMode="text" aria-autocomplete="none" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" value={announcementDraft} onChange={e => { setAnnouncementDraft(e.target.value); if (!announcementSpeaking) setAnnouncementStatus(''); }} onKeyDown={e => { if (e.key === 'Enter' && !announcementSpeaking) announceCustom(); }} placeholder="Announcement text…" className="bg-secondary" disabled={announcementSpeaking} /></div></div>{announcementStatus && <p className="text-[11px] text-muted-foreground">{announcementStatus}</p>}</div>
