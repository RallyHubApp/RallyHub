              ...
              <div className="border-t border-border p-4 sm:p-5">
                <div className="rounded-lg bg-secondary/30 p-4 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-[140px_180px_auto] gap-2 items-end">
                    <div><Label className="text-xs">Courts available now</Label><Input type="number" min="1" value={eventDayAdjust.courts} onChange={e=>{ setEventDayAdjustmentStatus(null); setEventDayAdjust(x=>({...x,courts:e.target.value})); }} placeholder={`${event.courts}`} className="mt-1 bg-secondary" disabled={eventDayAdjustmentBusy} /></div>
                    <div><Label className="text-xs">Minutes remaining</Label><Input type="number" min="1" value={eventDayAdjust.availableMinutes} onChange={e=>{ setEventDayAdjustmentStatus(null); setEventDayAdjust(x=>({...x,availableMinutes:e.target.value})); }} placeholder="e.g. 60" className="mt-1 bg-secondary" disabled={eventDayAdjustmentBusy} /></div>
                    <Button variant="outline" disabled={!canManageEvent || eventDayAdjustmentBusy} onClick={proposeEventDayAdjustment}>Preview Impact</Button>
                  </div>
