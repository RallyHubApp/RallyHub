            ...
            <div ref={hostBarAnchorRef} style={hostBarPinned ? { height: hostBarGeometry.height } : undefined}>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <div className="ml-auto flex flex-wrap items-center gap-2">
                  ...
                  {!['completed','archived'].includes(event.status) && (breakActive ? <Button size="sm" variant="destructive" disabled={!canManageEvent} onClick={timerRemaining > 0 ? endBreakEarly : advanceRound}>{timerRemaining > 0 ? `End Break → R${currentRound + 1}` : `Prepare R${currentRound + 1}`}</Button> : <Button size="sm" disabled={!canManageEvent || !currentRoundComplete} onClick={advanceRound}>{currentRoundComplete ? (scheduledBreakHere && currentRound < Math.max(...rounds) ? `Complete R${currentRound} → ${event.break_minutes}-min Break` : currentRound < Math.max(...rounds) ? `Complete Round ${currentRound}` : 'Finalise') : `${Math.max(0,currentMatches.length-currentRoundSavedCount)} score${Math.max(0,currentMatches.length-currentRoundSavedCount)===1?'':'s'} to save`}</Button>)}
                </div>
              </div>
