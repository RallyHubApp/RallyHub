...
export default function ClubChallengeView({ tournament, queryClient, isAdmin }) {
  ...
  return (
    <div data-testid="cc-root" className="space-y-4 print:space-y-0">
          ...
          {TABS.map(([id, label], index) => {
            ...
            return (
              <button key={id} data-testid={`cc-tab-${id}`} onClick={() => {
                ...
                if (id === 'live' && !['in_progress','paused','completed'].includes(event?.status)) { toast.info(`Approve the draw and start the ${INTERCLUB_EVENT_LABEL} first.`); return; }
                setTab(id);
      ...
      {tab === 'draw' && (
        <div className="space-y-4">
          {!matches.length ? <div className="glass rounded-xl p-8 text-center text-sm text-muted-foreground">Generate a draw from Teams & Ranking first.</div> : <>
            ...
            {event?.status === 'draw_approved' && <div className="rounded-xl border border-primary/25 bg-primary/5 p-3 space-y-3"><div><p className="text-xs font-bold">Hall sound & PA check before play</p><p className="text-[10px] text-muted-foreground">Set the laptop audio output to the JBL Charge 6. The built-in laptop microphone is the simplest V1 choice, but you can select a USB/external microphone if needed.</p>{paMicLabel && paActive && <p className="text-[10px] text-muted-foreground mt-1">Active mic: {paMicLabel}</p>}{paError && <p className="text-[10px] text-destructive mt-1">{paError}</p>}</div><div className="grid sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_auto_auto_auto] gap-2 items-end"><div><Label className="text-[10px]">Microphone</Label><Select value={selectedMicId} onValueChange={setSelectedMicId} disabled={paActive}><SelectTrigger className="mt-1 bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="default">System default microphone</SelectItem>{microphones.filter(mic => mic.deviceId && mic.deviceId !== 'default').map((mic, i) => <SelectItem key={mic.deviceId} value={mic.deviceId}>{mic.label || `Microphone ${i + 1}`}</SelectItem>)}</SelectContent></Select></div><Button variant="outline" className="min-h-11" onClick={refreshMicrophones} disabled={paActive}><RefreshCw className="w-4 h-4 mr-2" />Refresh Mics</Button><Button data-testid="cc-prestart-sound-check" variant="outline" className="min-h-11" onClick={() => unlockHallAudio({test:true})}>{audioReady ? 'Test Sound Again ✓' : 'Test Sound'}</Button><Button data-testid="cc-prestart-pa-test" className="min-h-11" variant={paActive ? 'destructive' : 'default'} disabled={paStarting} onClick={paActive ? stopPA : startPA}>{paActive ? <><MicOff className="w-4 h-4 mr-2" />Stop PA</> : <><Mic className="w-4 h-4 mr-2" />{paStarting ? 'Starting…' : 'Test PA Mic'}</>}</Button></div></div>}
      ...
      {tab === 'live' && (
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">Round at a Glance</p><p className="text-xs text-muted-foreground">The host should be able to answer “who is on, who is resting, who is next?” without leaving this screen.</p></div><Badge variant="outline">R{currentRound}</Badge></div><div className="grid md:grid-cols-2 xl:grid-cols-4 gap-2">{currentDisplayMatches.map(m=><div key={`now-${m.id}`} className="rounded-lg border border-primary/20 bg-primary/5 p-3"><p className="text-xs font-bold text-primary">Court {m.court_number} · NOW</p><p className="text-xs font-semibold mt-2">{(m.club_a_names||[]).join(' & ')}</p><p className="text-[10px] text-muted-foreground my-1">vs</p><p className="text-xs font-semibold">{(m.club_b_names||[]).join(' & ')}</p></div>)}</div>{currentSittingOut.length>0&&<div><p className="text-xs font-semibold">Resting this round</p><div className="flex flex-wrap gap-1.5 mt-2">{currentSittingOut.map(p=><Badge key={p.id} variant="outline">{p.display_name}</Badge>)}</div></div>}{nextDisplayMatches.length>0&&<div><p className="text-xs font-semibold">Up next · Round {currentRound+1}</p><div className="grid md:grid-cols-2 xl:grid-cols-4 gap-2 mt-2">{nextDisplayMatches.map(m=><div key={`next-${m.id}`} className="rounded-lg bg-secondary/50 p-2.5"><p className="text-[10px] font-bold">Court {m.court_number}</p><p className="text-[11px] mt-1">{(m.club_a_names||[]).join(' & ')} vs {(m.club_b_names||[]).join(' & ')}</p></div>)}</div></div>}</div>
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-4', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div><p className="text-xs font-semibold uppercase tracking-wider text-primary">RallyHub Audio & PA · V1</p><p className="text-sm font-semibold mt-1">Venue audio controller</p><p className="text-xs text-muted-foreground mt-1">Laptop microphone + RallyHub sounds and announcements → selected laptop audio output.</p></div>
                <Badge className={paActive ? 'bg-red-500/15 text-red-400' : audioReady ? 'bg-primary/10 text-primary' : ''} variant={paActive || audioReady ? 'default' : 'outline'}>{paActive ? 'PA LIVE' : audioReady ? 'AUDIO READY' : 'NOT TESTED'}</Badge>
              </div>
              <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end"><div><Label className="text-xs">Microphone</Label><Select value={selectedMicId} onValueChange={setSelectedMicId} disabled={paActive}><SelectTrigger className="mt-1 bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="default">System default microphone</SelectItem>{microphones.filter(mic => mic.deviceId && mic.deviceId !== 'default').map((mic, i) => <SelectItem key={mic.deviceId} value={mic.deviceId}>{mic.label || `Microphone ${i + 1}`}</SelectItem>)}</SelectContent></Select></div><Button variant="outline" className="h-10" onClick={refreshMicrophones} disabled={paActive}><RefreshCw className="w-4 h-4 mr-2" />Refresh Mics</Button></div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
                <Button data-testid="cc-pa-toggle" className="h-12" variant={paActive ? 'destructive' : 'default'} disabled={!canManageEvent || paStarting} onClick={paActive ? stopPA : startPA}>{paActive ? <><MicOff className="w-4 h-4 mr-2" />Stop PA</> : <><Mic className="w-4 h-4 mr-2" />{paStarting ? 'Starting PA…' : 'Start PA'}</>}</Button>
                <div className="rounded-lg bg-secondary/40 px-3 py-2"><Label className="text-xs">PA mic level · {Math.round(paGain * 100)}%</Label><input aria-label="RallyHub live PA microphone level" type="range" min="0" max="1.5" step="0.05" value={paGain} onChange={e => setPaGain(Number(e.target.value))} className="mt-2 w-full" /></div>
                <Button variant="outline" className="h-12" onClick={testVoice}>Test Sound</Button>
                <Button variant="outline" className="h-12" onClick={silenceAudio}><VolumeX className="w-4 h-4 mr-2" />Stop Audio</Button>
              </div>
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-3">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-2 rounded-lg bg-secondary/40 p-3">
                ...
                <div className="flex items-end"><Button variant="outline" className="w-full" disabled={!lastAnnouncement || voiceMode === 'off'} onClick={() => speak(lastAnnouncement, { signal:'warning' })}>Repeat Last</Button></div>
              </div>
              <p className="text-[10px] text-muted-foreground">Run Test Sound before play. RallyHub uses a local hall cue plus the device/browser’s default spoken voice at the selected volume, so sound does not depend on a Base44 call. Screen wake lock is requested while the timer runs.</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2"><Button data-testid="cc-timer-start-play" variant="outline" disabled={!canManageEvent} onClick={() => startPhase('play')}>Start Play</Button><Button variant="outline" disabled={!canManageEvent} onClick={() => startPhase('changeover')}>Changeover</Button><Button variant="outline" disabled={!canManageEvent || !event.include_break} onClick={() => startPhase('break')}>Break</Button><Button data-testid="cc-timer-pause" variant="outline" disabled={!canManageEvent || !timerState?.running} onClick={pauseTimer}>Pause</Button><Button data-testid="cc-timer-resume" variant="outline" disabled={!canManageEvent || !timerState || timerState.running || timerRemaining <= 0} onClick={resumeTimer}>Resume</Button><Button variant="outline" disabled={!canManageEvent} onClick={() => timerAction('add_minute')}>+1 min live</Button><Button variant="outline" disabled={!canManageEvent} onClick={resetTimer}>Reset</Button></div>
            </div>
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2"><Button className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId || !replacement.incomingName.trim()} onClick={applyReplacement}>Apply Replacement from Round {currentRound}</Button><Button variant="outline" className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId} onClick={withdrawWithoutReplacement}>Continue Short · No Replacement</Button></div>
            </div>
