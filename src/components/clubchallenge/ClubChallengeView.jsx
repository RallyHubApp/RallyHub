        ...
        <div className="space-y-4">
          {!event || !['in_progress','paused','completed','archived'].includes(event.status) ? <div className="rounded-xl border border-border bg-card/50 p-8 text-center text-sm text-muted-foreground">Approve the draw and start the {INTERCLUB_EVENT_LABEL} first.</div> : <>
            ...
            <div className="rounded-xl border border-primary/25 bg-card p-4 sm:p-5 space-y-4">
              ...
              <div className="rounded-lg bg-secondary/40 p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div><p className="text-xs font-semibold">This round</p><p className="text-[11px] text-muted-foreground">Adjust before play or while paused.</p></div>
                <div className="flex items-center justify-center gap-2"><Button variant="outline" size="icon" aria-label="Reduce this round by one minute" disabled={!canManageEvent || !!timerState?.running || preparedRoundMinutes <= 1} onClick={() => setRoundMinutes(preparedRoundMinutes - 1)}><Minus className="w-4 h-4" /></Button><p className="min-w-20 text-center text-2xl font-bold tabular-nums">{preparedRoundMinutes}:00</p><Button variant="outline" size="icon" aria-label="Add one minute to this round" disabled={!canManageEvent || !!timerState?.running || preparedRoundMinutes >= 60} onClick={() => setRoundMinutes(preparedRoundMinutes + 1)}><Plus className="w-4 h-4" /></Button></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                {timerState?.running ? <Button data-testid="cc-timer-pause" className="min-h-12" disabled={!canManageEvent} onClick={pauseTimer}>Pause</Button> : timerState && timerRemaining > 0 && timerState.phase !== 'idle' ? <Button data-testid="cc-timer-resume" className="min-h-12" disabled={!canManageEvent} onClick={resumeTimer}>Resume</Button> : <Button data-testid="cc-timer-start-play" className="min-h-12" disabled={!canManageEvent} onClick={() => startPhase('play')}><Play className="w-4 h-4 mr-2" />Start Play</Button>}
                <Button variant="outline" className="min-h-12" disabled={!canManageEvent} onClick={() => startPhase('changeover')}>Changeover</Button>
                {event.include_break && currentRound === event.break_after_round ? <Button variant="outline" className="min-h-12" disabled={!canManageEvent} onClick={() => startPhase('break')}>Start {event.break_minutes}-min Break</Button> : <Button variant="outline" className="min-h-12" disabled={!canManageEvent || !timerState?.running} onClick={() => timerAction('add_minute')}>+1 minute</Button>}
            ...
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-3">
              <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">Round {currentRound} Scores</p><p className="text-xs text-muted-foreground">Enter each court result as it comes in.</p></div><Badge variant="outline">{currentMatches.filter(m => ['completed','draw','retired','forfeit','abandoned','not_played'].includes(m.status)).length}/{currentMatches.length} saved</Badge></div>
            ...
            <div data-testid="cc-audio-pa-controller" className={cn('rounded-xl border p-4 sm:p-5 space-y-3', paActive ? 'border-red-500/60 bg-red-500/5' : 'border-border bg-card')}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p className="text-sm font-semibold">PA & Announcements</p><p className="text-xs text-muted-foreground">Everything you need during play, without leaving RallyHub.</p></div><div className="flex items-center gap-2"><Badge className={paActive ? 'bg-red-500/15 text-red-400' : audioReady ? 'bg-primary/10 text-primary' : ''} variant={paActive || audioReady ? 'default' : 'outline'}>{paActive ? 'PA LIVE' : audioReady ? 'AUDIO READY' : 'NOT TESTED'}</Badge><Button variant="outline" size="sm" onClick={silenceAudio}><VolumeX className="w-4 h-4 mr-2" />Stop Audio</Button></div></div>
              <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-2 items-end">
                <div><Label className="text-xs">Microphone</Label><Select value={selectedMicId} onValueChange={setSelectedMicId} onOpenChange={open => { if (open && !paActive) refreshMicrophones(); }} disabled={paActive}><SelectTrigger className="mt-1 bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="default">System default microphone</SelectItem>{microphones.filter(mic => mic.deviceId && mic.deviceId !== 'default').map((mic, i) => <SelectItem key={mic.deviceId} value={mic.deviceId}>{mic.label || `Microphone ${i + 1}`}</SelectItem>)}</SelectContent></Select></div>
                ...
                <div className="rounded-lg bg-secondary/40 px-3 py-2"><Label className="text-xs">Mic · {Math.round(paGain * 100)}%</Label><input aria-label="RallyHub live PA microphone level" type="range" min="0" max="1.5" step="0.05" value={paGain} onChange={e => setPaGain(Number(e.target.value))} className="mt-2 w-full" /></div>
                <div className="rounded-lg bg-secondary/40 px-3 py-2"><Label className="text-xs">RallyHub sound · {Math.round(hallVolume * 100)}%</Label><input aria-label="RallyHub live sound volume" type="range" min="0" max="1" step="0.05" value={hallVolume} onChange={e => setHallVolume(Number(e.target.value))} className="mt-2 w-full" /></div>
              </div>
              ...
              <div className="space-y-2"><p className="text-xs font-semibold">Voice announcements</p><div className="flex flex-col md:flex-row gap-2"><Button className="w-full md:w-36 md:shrink-0" variant="outline" disabled={!announcementDraft.trim() || paActive || announcementSpeaking} onClick={announceCustom}><Megaphone className="w-4 h-4 mr-2" />{announcementSpeaking ? 'Speaking…' : 'Announce'}</Button><Input type="text" name="rallyhub-announcement-text" autoComplete="off" inputMode="text" aria-autocomplete="none" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" value={announcementDraft} onChange={e => { setAnnouncementDraft(e.target.value); if (!announcementSpeaking) setAnnouncementStatus(''); }} onKeyDown={e => { if (e.key === 'Enter' && !announcementSpeaking) announceCustom(); }} placeholder="Announcement text…" className="bg-secondary" disabled={announcementSpeaking} /><Button variant="outline" className="w-full md:w-32 md:shrink-0" disabled={!lastAnnouncement || paActive || announcementSpeaking} onClick={() => speak(lastAnnouncement, { signal:'warning' })}>Repeat Last</Button></div>{announcementStatus && <p className="text-[11px] text-muted-foreground">{announcementStatus}</p>}</div>
            </div>

            <details className="rounded-xl border border-border bg-card overflow-hidden">
              <summary className="cursor-pointer list-none p-4 sm:p-5 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">Event Changes</p><p className="text-xs text-muted-foreground">Replacement, late arrival, court or time changes.</p></div><ChevronDown className="w-4 h-4 text-muted-foreground" /></summary>
              <div className="border-t border-border p-4 sm:p-5 space-y-4">
                <div className="rounded-lg bg-secondary/30 p-4 space-y-3"><div><p className="text-sm font-semibold">Withdrawal / Injury / Replacement</p><p className="text-xs text-muted-foreground">Completed results stay unchanged; only future fixtures are updated.</p></div><div className="grid md:grid-cols-2 lg:grid-cols-5 gap-2"><Select value={replacement.outgoingId} onValueChange={v => setReplacement(r => ({ ...r, outgoingId: v }))}><SelectTrigger className="bg-secondary"><SelectValue placeholder="Outgoing player" /></SelectTrigger><SelectContent>{participants.filter(p => p.status === 'active').map(p => <SelectItem key={p.id} value={p.id}>{p.display_name} · {p.side === 'club_a' ? event.club_a_name : event.club_b_name}</SelectItem>)}</SelectContent></Select><Input value={replacement.incomingName} onChange={e => setReplacement(r => ({ ...r, incomingName: e.target.value }))} placeholder="Replacement player name" className="bg-secondary" /><Select value={replacement.incomingGender || 'inherit'} onValueChange={v => setReplacement(r => ({ ...r, incomingGender: v === 'inherit' ? '' : v }))}><SelectTrigger className="bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="inherit">Inherit gender</SelectItem><SelectItem value="Male">Male</SelectItem><SelectItem value="Female">Female</SelectItem></SelectContent></Select><Select value={replacement.status} onValueChange={v => setReplacement(r => ({ ...r, status: v }))}><SelectTrigger className="bg-secondary"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="withdrawn">Withdrawn</SelectItem><SelectItem value="injured">Injured</SelectItem><SelectItem value="replaced">Replaced</SelectItem></SelectContent></Select><Input value={replacement.reason} onChange={e => setReplacement(r => ({ ...r, reason: e.target.value }))} placeholder="Reason / note" className="bg-secondary" /></div><div className="flex flex-col sm:flex-row gap-2"><Button className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId || !replacement.incomingName.trim()} onClick={applyReplacement}>Apply Replacement from Round {currentRound}</Button><Button variant="outline" className="w-full sm:w-auto" disabled={!canManageEvent || !replacement.outgoingId} onClick={withdrawWithoutReplacement}>Continue Short · No Replacement</Button></div></div>
                <div className="grid lg:grid-cols-2 gap-3"><div className="rounded-lg bg-secondary/30 p-4 space-y-3"><div><p className="text-sm font-semibold">Late Arrival</p><p className="text-xs text-muted-foreground">Set the first round the player is available.</p></div><div className="grid sm:grid-cols-[1fr_120px_auto] gap-2"><Select value={lateArrival.participantId} onValueChange={v => setLateArrival(x => ({...x, participantId:v}))}><SelectTrigger className="bg-secondary"><SelectValue placeholder="Player" /></SelectTrigger><SelectContent>{participants.filter(p=>!['withdrawn','injured','replaced'].includes(p.status)).map(p=><SelectItem key={p.id} value={p.id}>{p.display_name}</SelectItem>)}</SelectContent></Select><Input type="number" min={currentRound||1} value={lateArrival.round} onChange={e=>setLateArrival(x=>({...x,round:e.target.value}))} className="bg-secondary" /><Button variant="outline" disabled={!canManageEvent || !lateArrival.participantId} onClick={applyLateArrival}>Set Round</Button></div></div><div className="rounded-lg bg-secondary/30 p-4 space-y-3"><div><p className="text-sm font-semibold">Court / Time Change</p><p className="text-xs text-muted-foreground">Review the impact before confirming any change.</p></div><div className="grid sm:grid-cols-[120px_140px_auto] gap-2"><Input type="number" min="1" value={eventDayAdjust.courts} onChange={e=>setEventDayAdjust(x=>({...x,courts:e.target.value}))} placeholder={`${event.courts} courts`} className="bg-secondary" /><Input type="number" min="1" value={eventDayAdjust.availableMinutes} onChange={e=>setEventDayAdjust(x=>({...x,availableMinutes:e.target.value}))} placeholder="Minutes left" className="bg-secondary" /><Button variant="outline" disabled={!canManageEvent} onClick={proposeEventDayAdjustment}>Calculate</Button></div>{eventDayProposal && <div className="rounded-lg bg-secondary/50 p-3 text-xs space-y-2"><p><strong>Review:</strong> {eventDayProposal.unresolved - eventDayProposal.dropIds.length} matches retained · {eventDayProposal.changes.length} positions change · {eventDayProposal.dropIds.length} Not Played.</p><p className="text-muted-foreground">Completed results are locked. Event Pack becomes OUT OF DATE.</p><div className="flex gap-2"><Button size="sm" disabled={!canManageEvent} onClick={confirmEventDayAdjustment}>Confirm Changes</Button><Button size="sm" variant="outline" onClick={()=>setEventDayProposal(null)}>Cancel</Button></div></div>}</div></div>
              </div>
            </details>
            {!['completed','archived'].includes(event.status) && <Button className="w-full h-11" disabled={!canManageEvent} onClick={advanceRound}>{currentRound < Math.max(...rounds) ? `Complete Round ${currentRound} & Go to Round ${currentRound + 1}` : <><Trophy className="w-4 h-4 mr-2" />Finalise {INTERCLUB_EVENT_LABEL}</>}</Button>}
          </>}
        </div>
      )}
