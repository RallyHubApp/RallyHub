...
function VotingPrompt({ votingUrl, countdown, compact=false }){ if(!votingUrl) return null; return <div className={compact?'flex items-center justify-center gap-4 text-center':'flex flex-col sm:flex-row items-center justify-center gap-4 text-center'}><div><p className="text-sm sm:text-lg font-black uppercase tracking-[.16em] text-primary">Players of the Tournament voting open</p><p className="mt-1 text-2xl sm:text-4xl font-black tabular-nums">{countdown}</p><p className="mt-1 text-xs sm:text-sm text-muted-foreground">Scan the QR or tap Vote now.</p><a href={votingUrl} className="mt-2 inline-flex min-h-10 items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-bold text-primary-foreground shadow-sm">Vote now</a><a href={votingUrl} className="block mt-1 text-[10px] sm:text-xs text-primary underline underline-offset-2 break-all">Open voting link</a></div><div className="rounded-xl bg-white p-2 shadow-sm"><QRCodeSVG value={votingUrl} size={compact?86:104}/></div></div>; }
