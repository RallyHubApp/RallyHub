...
function CourtScore({match,session,token,clientId,onSaved}){
 ...
 const submit=async()=>{if(!locked)return;if(session.scoring_mode==='timed'&&a!==''&&b!==''&&Number(a)===Number(b)&&!serving){setError('Choose which team was serving at the horn before saving a tied timed game.');return;}setSaving(true);setError('');setNotice('');try{const r=await base44.functions.invoke('kotcScorer',{action:saved?'correct':'save',token,matchId:match.id,clientId,expectedRevision:effectiveMatch.revision,teamAScore:Number(a),teamBScore:Number(b),servingSideAtHorn:Number(a)===Number(b)?serving:undefined});if(r.data?.error)throw new Error(r.data.error);const savedMatch={...(r.data?.match||effectiveMatch),can_correct:true,lock_status:'free'};setLocalSaved(savedMatch);setLocked(false);setEditing(false);setNotice(r.data?.message||'Score saved');onSaved?.();}catch(e){setError(msg(e));onSaved?.();}finally{setSaving(false);}};
