...
function CourtScore({match,session,token,clientId,onSaved}){
 const [a,setA]=useState(match.team_a_score??''),[b,setB]=useState(match.team_b_score??''),[serving,setServing]=useState('A'),[saving,setSaving]=useState(false),[editing,setEditing]=useState(false),[locked,setLocked]=useState(match.lock_status==='mine'),[error,setError]=useState(''),[notice,setNotice]=useState('');
 ...
 useEffect(()=>{if(!editing){setA(match.team_a_score??'');setB(match.team_b_score??'');}setLocked(match.lock_status==='mine');},[match.id,match.revision,match.team_a_score,match.team_b_score,match.lock_status]);
 ...
 const claim=async(forCorrection=false)=>{setError('');setNotice('');try{const r=await base44.functions.invoke('kotcScorer',{action:'claim',token,matchId:match.id,clientId});if(r.data?.error)throw new Error(r.data.error);setLocked(true);setEditing(true);setA(match.team_a_score??'');setB(match.team_b_score??'');setNotice(forCorrection?'Score unlocked for correction':'Court locked to this device');}catch(e){setError(msg(e));await onSaved();}};
 ...
 const submit=async()=>{if(!locked)return;setSaving(true);setError('');setNotice('');try{const r=await base44.functions.invoke('kotcScorer',{action:saved?'correct':'save',token,matchId:match.id,clientId,expectedRevision:match.revision,teamAScore:Number(a),teamBScore:Number(b),servingSideAtHorn:Number(a)===Number(b)?serving:undefined});if(r.data?.error)throw new Error(r.data.error);setLocked(false);setEditing(false);setNotice(r.data?.message||'Score saved');await onSaved();}catch(e){setError(msg(e));await onSaved();}finally{setSaving(false);}};
 ...
 return <div className={`rounded-xl border bg-card p-4 space-y-3 ${locked?'ring-2 ring-primary/30 border-primary/50':''}`} data-testid={`scorer-court-${match.court}`}>
  ...
  {editing&&<div className="grid grid-cols-2 gap-2"><Button className="min-h-12" onClick={submit} disabled={saving||a===''||b===''}>{saving?'Saving…':saved?'Save Updated Score':'Save Result'}</Button><Button variant="outline" className="min-h-12" onClick={release} disabled={saving}><X className="w-4 h-4 mr-1"/>Cancel</Button></div>}
