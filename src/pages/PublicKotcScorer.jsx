...
function CourtScore({match,session,token,clientId,onSaved}){
 const [a,setA]=useState(match.team_a_score??''),[b,setB]=useState(match.team_b_score??''),[serving,setServing]=useState(''),[saving,setSaving]=useState(false),[editing,setEditing]=useState(false),[locked,setLocked]=useState(match.lock_status==='mine'),[error,setError]=useState(''),[notice,setNotice]=useState(''),[localSaved,setLocalSaved]=useState(null);
 ...
 useEffect(()=>{if(!editing){setA(match.team_a_score??'');setB(match.team_b_score??'');setServing(match.serving_side_at_horn||'');}if(localSaved&&Number(match.revision||0)>=Number(localSaved.revision||0))setLocalSaved(null);setLocked(match.lock_status==='mine');},[match.id,match.revision,match.team_a_score,match.team_b_score,match.serving_side_at_horn,match.lock_status]);
 ...
 const claim=async(forCorrection=false)=>{setError('');setNotice('');try{await scorerInvoke({action:'claim',token,matchId:match.id,clientId},3);setLocked(true);setEditing(true);setA(match.team_a_score??'');setB(match.team_b_score??'');setServing(match.serving_side_at_horn||'');setNotice(forCorrection?`Court ${match.court} ready for correction`:`Court ${match.court} ready — enter the score`);}catch(e){setError(isRateLimit(e)?'RallyHub is busy. Tap Score This Court again.':msg(e));await onSaved();}};
 ...
 return <div className={`rounded-xl border bg-card p-4 space-y-3 ${locked?'ring-2 ring-primary/30 border-primary/50':''}`} data-testid={`scorer-court-${match.court}`}>
  ...
  <div className="grid grid-cols-[1fr_84px] gap-3 items-center"><span>{match.team_a.join(' & ')}</span><Input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} className="h-12 text-center text-lg font-bold" value={a} disabled={!canInput||saving} onChange={e=>setA(e.target.value.replace(/\D/g,'').slice(0,2))}/></div>
  <div className="grid grid-cols-[1fr_84px] gap-3 items-center"><span>{match.team_b.join(' & ')}</span><Input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} className="h-12 text-center text-lg font-bold" value={b} disabled={!canInput||saving} onChange={e=>setB(e.target.value.replace(/\D/g,'').slice(0,2))}/></div>
  ...
  {canCorrect&&!editing&&!blocked&&<Button variant="outline" className="w-full min-h-11" onClick={()=>claim(true)}><Pencil className="w-4 h-4 mr-2"/>Undo / Update Score</Button>}
...
export default function PublicKotcScorer(){
 ...
 const load=async()=>{try{const r=await scorerInvoke({action:'state',token,clientId},2);setData(r.data);setError('');setOffline(false);return r.data;}catch(e){if(isRateLimit(e)&&dataRef.current)return dataRef.current;if(dataRef.current)setOffline(true);else setError(msg(e));return null;}};
 useEffect(()=>{load();const p=setInterval(load,5000),t=setInterval(()=>setNow(Date.now()),1000);const off=()=>setOffline(true),on=()=>{setOffline(false);load();};window.addEventListener('offline',off);window.addEventListener('online',on);window.addEventListener('focus',load);return()=>{clearInterval(p);clearInterval(t);window.removeEventListener('offline',off);window.removeEventListener('online',on);window.removeEventListener('focus',load);};},[token]);
