...
export default function About(){
 ...
 const uploadApproved=async(purpose,file)=>{ if(!file)return; setUploading(purpose); try { const res=await base44.functions.invoke('secureCreditAction',{ action:'upload_image', purpose, file }); const url=res?.data?.file_url||res?.file_url; if(!url) throw new Error(res?.data?.error||'No file URL returned'); setUploaded(v=>({...v,[purpose]:url})); } catch(e){ alert('Upload failed: '+(e?.response?.data?.error||e?.message||e)); } finally { setUploading(''); } };
 ...
 <div className="min-h-screen bg-white font-sans text-[#07184c]"><Header/>{user?.role==='admin'&&<div className="mx-auto my-3 max-w-[964px] rounded-lg border-2 border-[#08a65a] bg-[#f4fbf7] p-4"><div className="font-black text-[#07184c]">TEMPORARY SUPER ADMIN — SIGNED-OFF ABOUT VISUAL</div><div className="mt-1 text-sm text-[#344a72]">Upload the single full-page signed-off About visual. This is the locked master reference; the genuine Conall and Brian photograph remains the approved founder substitution.</div><div className="mt-3"><label className="inline-block rounded-md bg-[#075287] px-4 py-2 text-sm font-bold text-white cursor-pointer">{uploading==='about_master'?'Uploading master visual…':uploaded.about_master?'✓ Signed-off About visual uploaded':'Upload signed-off About visual'}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e=>uploadApproved('about_master',e.target.files?.[0])}/></label></div></div>}<main>
  ...
  <section className="bg-white px-[30px] pb-[24px]"><img src="/assets/about-locked/about-footer-approved.png?v=20260921" alt="Join the RallyHub journey — let’s grow the game together" className="mx-auto block h-auto w-full max-w-[964px]"/></section>
 </main></div></>;
}