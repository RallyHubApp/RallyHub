...
export default function About(){
 ...
 <div className="min-h-screen bg-white font-sans text-[#07184c]"><Header/>{user?.role==='admin'&&<div className="mx-auto max-w-[1024px] border-b border-amber-200 bg-amber-50 px-6 py-3 text-sm"><div className="flex flex-wrap items-center gap-3"><strong>Temporary Super Admin: Founder photo</strong><label className="cursor-pointer rounded-full bg-[#07528a] px-5 py-2 font-bold text-white">Upload original founder photo<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={uploadFounder}/></label><span>{founderUploadStatus}</span></div></div>}<main>
  <section className="bg-[#f7fbfc]"><div className="mx-auto grid max-w-[1024px] md:grid-cols-[42%_58%]">
   ...
   <div className="relative min-h-[437px] overflow-hidden bg-[#dce8e8]"><img src={HERO} alt="Pickleball players on court" className="absolute inset-0 h-full w-full object-cover object-center"/><div className="absolute bottom-[20px] right-[18px] rotate-[-6deg] bg-[#07184c]/95 px-[20px] py-[12px] text-right text-[18px] font-semibold italic leading-[1.25] text-white">Good People.<br/>Great Games.<div className="ml-auto mt-[6px] h-[3px] w-[92px] bg-[#83cf3d]"/></div></div>
  </div></section>
  ...
  <section className="mx-auto grid max-w-[1024px] gap-[28px] px-[30px] pb-[19px] md:grid-cols-[47%_53%]">
   <div className="relative h-[340px] overflow-hidden rounded-[5px] bg-[#e9eef0]"><img src="/assets/rallyhub-founders-hq.webp?v=20260921e" alt="Conall and Brian Moore" className="absolute inset-0 h-full w-full object-contain object-center [image-rendering:auto]"/><div className="absolute bottom-0 left-[10px] rounded-t-[5px] bg-[#075287] px-[12px] py-[8px] text-white"><div className="text-[11px] font-bold">Conall and Brian Moore</div><div className="text-[9px]">Founders, RallyHub</div></div></div>
  ...
  <section className="bg-white px-[30px] pb-[24px]"><div className="relative mx-auto min-h-[212px] max-w-[964px] overflow-hidden rounded-[6px] bg-[#073d65] text-white"><img src={CTA_COMMUNITY} alt="Pickleball community" className="absolute inset-y-0 left-0 h-full w-[38%] object-cover"/><img src={CTA_SCENIC} alt="Irish landscape" className="absolute inset-y-0 right-0 h-full w-[44%] object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,61,101,0)_0%,#073d65_34%,#073d65_68%,rgba(7,61,101,.20)_100%)]"/><div className="relative z-10 grid min-h-[212px] md:grid-cols-[34%_42%_24%]"><div/><div className="flex items-center px-[20px] py-[25px]"><div><div className="text-[9px] font-black tracking-[.12em] text-[#65d77d]">JOIN THE JOURNEY</div><h2 className="mt-[7px] text-[25px] font-black">Let’s grow the game together</h2><p className="mt-[6px] text-[11px] leading-[1.45] text-white/95">Whether you’re a player, club, organiser or just curious about pickleball, we’d love to hear from you.</p><div className="mt-[15px] flex flex-wrap gap-[10px]"><Link to="/contact" className="rounded-full bg-[#08a65a] px-[28px] py-[11px] text-[10px] font-bold">Get in Touch</Link><a href="https://wa.me/353878100333" target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full border border-white px-[17px] py-[10px] text-[10px] font-bold"><MessageCircle className="h-5 w-5"/>Contact Brian on WhatsApp</a></div></div></div><div className="relative hidden md:block"><div className="absolute left-1/2 top-1/2 flex h-[154px] w-[154px] -translate-x-1/2 -translate-y-1/2 rotate-[-5deg] items-center justify-center rounded-full border-[3px] border-white text-center text-[17px] font-semibold italic leading-[1.3]">More<br/>People<br/>More Places<br/><span className="text-[#b7e83e]">Brighter Futures</span></div></div></div></div></section>
 </main></div></>;
}