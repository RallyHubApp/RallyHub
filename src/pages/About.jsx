...
export default function About(){
 ...
 <div className="min-h-screen bg-white font-sans text-[#07184c]"><Header/><main>
  ...
  <section className="bg-white px-[30px] pb-[24px]"><div className="relative mx-auto min-h-[214px] max-w-[964px] overflow-hidden rounded-[6px] bg-[#073d65] text-white"><img src={CTA_COMMUNITY} alt="Pickleball community" className="absolute inset-y-0 left-0 h-full w-[38%] object-cover"/><img src={CTA_SCENIC} alt="Irish landscape" className="absolute inset-y-0 right-0 h-full w-[44%] object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,61,101,0)_0%,#073d65_34%,#073d65_68%,rgba(7,61,101,.20)_100%)]"/><div className="relative z-10 grid min-h-[225px] md:grid-cols-[34%_42%_24%]"><div/><div className="flex items-center px-[20px] py-[25px]"><div><div className="text-[9px] font-black tracking-[.12em] text-[#65d77d]">JOIN THE JOURNEY</div><h2 className="mt-[7px] text-[25px] font-black">Let’s grow the game together</h2><p className="mt-[6px] text-[11px] leading-[1.45] text-white/95">Whether you’re a player, club, organiser or just curious about pickleball, we’d love to hear from you.</p><div className="mt-[15px] flex flex-wrap gap-[10px]"><Link to="/contact" className="rounded-full bg-[#08a65a] px-[28px] py-[11px] text-[10px] font-bold">Get in Touch</Link><a href="https://wa.me/353878100333" target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full border border-white px-[17px] py-[10px] text-[10px] font-bold"><MessageCircle className="h-5 w-5"/>Contact Brian on WhatsApp</a></div></div></div><div className="relative hidden md:block"><div className="absolute left-1/2 top-1/2 flex h-[154px] w-[154px] -translate-x-1/2 -translate-y-1/2 rotate-[-5deg] items-center justify-center rounded-full border-[3px] border-white text-center text-[17px] font-semibold italic leading-[1.3]">More<br/>People<br/>More Places<br/><span className="text-[#b7e83e]">Brighter Futures</span></div></div></div></div></section>
 </main></div></>;
}