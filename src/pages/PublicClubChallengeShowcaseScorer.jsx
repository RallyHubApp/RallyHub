...
export default function PublicClubChallengeShowcaseScorer(){
  ...
  if(error&&!data) return <div className="min-h-screen bg-background text-foreground grid place-items-center p-5 text-center"><AppearanceQuickButton className="fixed right-3 top-3 z-50 h-10 px-2 sm:px-3"/><div className="max-w-sm"><RallyHubPublicBrand moduleName={INTERCLUB_MODULE_NAME} pageLabel="Showcase Scorer"/><WifiOff className="mx-auto mt-5 h-8 w-8"/><h1 className="mt-3 font-bold">Showcase scorer link unavailable</h1><p className="mt-2 text-sm text-muted-foreground">{error}</p></div></div>;
  if(!data) return <div className="min-h-screen bg-background text-foreground grid place-items-center"><AppearanceQuickButton className="fixed right-3 top-3 z-50 h-10 px-2 sm:px-3"/><div className="text-center"><RallyHubPublicBrand moduleName={INTERCLUB_MODULE_NAME} pageLabel="Showcase Scorer"/><RefreshCw className="mx-auto mt-5 h-7 w-7 animate-spin"/></div></div>;

  const {event,match}=data;
  const eventClubs=[
    {id:'club_a',name:event.club_a_name,logo_url:event.club_a_logo_url,primary_colour:event.club_a_primary_colour,secondary_colour:event.club_a_secondary_colour},
    {id:'club_b',name:event.club_b_name,logo_url:event.club_b_logo_url,primary_colour:event.club_b_primary_colour,secondary_colour:event.club_b_secondary_colour},
  ];
  ...
  return <div className="min-h-screen bg-background text-foreground p-3 sm:p-5"><AppearanceQuickButton className="fixed right-3 top-3 z-50 h-10 px-2 sm:px-3"/>
    <div className="mx-auto max-w-xl space-y-4">
      <header className="rounded-2xl border border-border bg-card p-4 text-center">
        <RallyHubPublicBrand moduleName={INTERCLUB_MODULE_NAME} pageLabel="Showcase Scorer" clubs={eventClubs}/>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        ].map(([side,club,names,score])=>{const brand=side==='a'?eventClubs[0]:eventClubs[1];return <div key={side} className="rounded-2xl border border-border bg-card p-3 text-center" style={{borderTopWidth:6,borderTopColor:brand.primary_colour||undefined,borderBottomWidth:2,borderBottomColor:brand.secondary_colour||brand.primary_colour||undefined}}>
