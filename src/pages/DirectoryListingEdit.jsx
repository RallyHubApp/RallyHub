...
export async function prepareLogoDraft(file) {
  ...
  try {
    ...
    if (trimTransparent || trimWhite) {
      for (let y = 0; y < scanHeight; y += 1) {
        for (let x = 0; x < scanWidth; x += 1) {
          ...
          const isBackground = trimTransparent ? a < 24 : (a > 220 && r > 240 && g > 240 && b > 240);
          if (!isBackground) {
    ...
    if (!foundContent) {
      return { file, url: originalUrl, width: image.naturalWidth, height: image.naturalHeight, zoom: 1, offsetX: 0, offsetY: 0, autoTrimmed: false };
    }
...
export default function DirectoryListingEdit() {
  ...
  const chooseLogo = async file => {
    ...
    try {
      ...
      setError(err?.message || 'RallyHub could not prepare that logo. Please try another image.');
    }
  };
        ...
        <main className="container mx-auto px-4 py-7 max-w-6xl">
          ...
          {isLoadingAuth || !authChecked || loadingListing ? (
            ...
            <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap gap-2 shrink-0">
                    ...
                    {user?.role === 'admin' && !isClaimed && <>
                      ...
                      {form?.contact?.email && <Button type="button" variant={form?.contact?.phone ? 'outline' : 'default'} className="gap-2" onClick={sendClaimInvite} disabled={saving || inviting || dirty} title={dirty ? 'Save changes before sending the claim invitation' : undefined}>
                        ...
                        {inviting ? 'Sending email…' : dirty ? 'Save before email' : 'Email claim invitation'}
                      </Button>}
                    </>}
              ...
              {showEnhancements && <DirectorySpondPanel listingSlug={slug} clubName={baseClub?.name || ''} onImport={importSpondDirectoryData} onSave={save} saveBusy={saving} hasUnsavedChanges={dirty} saved={saved} />}
                ...
                {(form.venues || []).map((venue, index) => (
                  <div key={venue.id || index} className="rounded-xl border border-border bg-background/30 p-4 sm:p-5 space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1"><Label>Eircode / postcode</Label><Input value={venue.eircode || ''} onChange={e => setVenue(index, 'eircode', e.target.value.toUpperCase())} autoComplete="postal-code" /><p className="text-xs text-muted-foreground">Address and Eircode/postcode are used to position this venue automatically on the all-Ireland club map when you save.</p></div>
                      ...
                      <div className="space-y-1"><Label>Google Maps link <span className="text-muted-foreground">(optional)</span></Label><Input value={venue.mapUrl || ''} onChange={e => setVenue(index, 'mapUrl', e.target.value)} placeholder="Paste a full Google Maps link if you have one" /><p className="text-xs text-muted-foreground">You do not need this link. RallyHub can use the address/Eircode, or you can set the pin directly on the map below.</p></div>
                    </div>
              ...
              <section id="sessions" className="glass rounded-2xl p-6 space-y-4 scroll-mt-24">
                <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><CalendarDays className="w-5 h-5 text-primary" /><h2 className="text-xl font-bold">Weekly sessions</h2></div><p className="text-sm text-muted-foreground mt-1">Duplicate a similar session to save retyping. RallyHub automatically orders sessions Monday to Sunday, then by start time.</p></div><Button type="button" variant="outline" size="sm" onClick={addSession} disabled={!form.venues?.length} className="gap-1" data-testid="directory-add-session"><Plus className="w-4 h-4" /> Add blank session</Button></div>
