...
export async function requireConfiguredEmailTransport(base44: any, scope: EmailScope) {
  ...
  if (config.status !== 'configured') {
    throw new Error(`Email transport for ${scope.scopeType}:${scope.purpose} is not active yet (${config.sender_email || 'sender pending'}).`);
  }
  return config;
}
...
function gmailRawEmail({to,senderEmail,senderName,replyTo,subject,textBody,htmlBody}:{to:string;senderEmail:string;senderName:string;replyTo?:string|null;subject:string;textBody:string;htmlBody?:string|null}) {
  const boundary = `rallyhub_${crypto.randomUUID().replace(/-/g, '')}`;
  const headers = [`From: ${mimeHeader(senderName)} <${senderEmail}>`,`To: ${to}`,`Reply-To: ${replyTo || senderEmail}`,`Subject: ${mimeHeader(subject)}`,'MIME-Version: 1.0'];
  const mime = htmlBody ? [...headers,`Content-Type: multipart/alternative; boundary="${boundary}"`,'',`--${boundary}`,'Content-Type: text/plain; charset=UTF-8','Content-Transfer-Encoding: 8bit','',textBody,`--${boundary}`,'Content-Type: text/html; charset=UTF-8','Content-Transfer-Encoding: 8bit','',htmlBody,`--${boundary}--`,''].join('\r\n') : [...headers,'Content-Type: text/plain; charset=UTF-8','Content-Transfer-Encoding: 8bit','',textBody].join('\r\n');
