...
Deno.serve(async(req)=>{try{
  ...
  const match=(await base44.asServiceRole.entities.KotcMatch.filter({id:matchId,session_id:session.id}))?.[0];if(!match)return Response.json({error:'Match not found'},{status:404});
...
}catch(error){return Response.json({error:(error as any)?.message||'Unexpected KOTC score error'},{status:500});}});
