...
async function refreshKotcAggregates(base44:any,session:any){
 ...
 const [allClubSessions,allTournaments]=await Promise.all([base44.asServiceRole.entities.KotcSession.filter({tenant_id:session.tenant_id,club_id:session.club_id},'-created_date',500),base44.asServiceRole.entities.Tournament.filter({tenant_id:session.tenant_id},'-created_date',500)]);const tournamentById=new Map((allTournaments||[]).map((t:any)=>[String(t.id),t]));const clubSessions=(allClubSessions||[]).filter((s:any)=>['completed','finalised'].includes(s.status)&&s.exclude_from_aggregates!==true&&tournamentById.get(String(s.tournament_id))?.counts_toward_leaderboard!==false);const validSessionIds=new Set(clubSessions.map((s:any)=>String(s.id)));
