...
async function sendClaimInviteEmail(base44, { user, listing, contactEmail, contactName, claimUrl, accessRole = 'owner', customText = '', customSubject = '' }) {
  ...
  const sharedStory = `RallyHub started as a father-and-son project between Conall and me, originally to solve some of the practical things we needed for Clare Pickleball. It has grown from there, and the first public phase is the RallyHub Club Directory — helping players find clubs, venues and regular sessions around Ireland.`;
  const ownerBody = `Hi ${recipientName},\n\nI’m getting in touch because I’ve put together a free RallyHub Directory listing for ${listing.name} as part of a wider effort to improve information on pickleball clubs around Ireland, following David Molloy’s request for help updating the national club map.\n\n${sharedStory}\n\nI’ve already created the ${listing.name} listing, so most of the work is done. I’d simply like you to have a look, claim the listing and correct or add anything that needs updating.\n\nOnce verified, you’ll become the Primary Directory Owner for ${listing.name}, which means you can manage the club’s public Directory information directly.\n\nThe Directory listing is completely free. There is no subscription, no catch and no obligation to use any other RallyHub services.\n\nRallyHub is also developing other optional club tools around session management, King of the Court, tournaments and events, but those are separate from your free Directory listing.\n\nYour secure claim link:\n${claimUrl}\n\nThe link is personal to you, can only be used once and expires after 72 hours.\n\nIf you’d like to have a quick look at RallyHub first:\nAbout RallyHub: https://rallyhub.ie/about\n1-page Directory Explainer: https://rallyhub.ie/directory/story\n\nClub Guide & Help: https://rallyhub.ie/directory/help\nQuick Start Guide: https://rallyhub.ie/directory/quick-start\n\nThere is also a Feedback area inside RallyHub, and I’m always happy to hear suggestions about what would genuinely be useful to clubs.\n\nIf you have any difficulty claiming the listing, just WhatsApp or call me.\n\nYours in sport,\nBrian Moore\n087 810 0333\nhttps://rallyhub.ie`;
  const editorBody = `Hi ${recipientName},\n\nYou’ve been invited as a Directory Editor for ${listing.name} on RallyHub. This gives you access to help maintain the club’s public Directory information only. It does not give access to RallyHub Club, tournaments, players or club administration.\n\nYour secure editor link:\n${claimUrl}\n\nThe link is personal to you, can only be used once and expires after 72 hours.\n\nAbout RallyHub: https://rallyhub.ie/about\nClub Guide & Help: https://rallyhub.ie/directory/help\n\nYours in sport,\nBrian Moore\n087 810 0333\nhttps://rallyhub.ie`;
...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'send_prepared_invitation_email') {
      ...
      if (user.role !== 'admin' && String(invitation.created_by_user_id || '') !== String(user.id)) return Response.json({ error: 'You cannot send this invitation' }, { status: 403 });
    ...
    if (action === 'invite_editor') {
      ...
      if (inviteChannel === 'email' && !body.prepareOnly) {
        const result = await sendClaimInviteEmail(base44, {
    ...
    if (action === 'list_admin') {
      ...
      const [claims, accesses, listingRequests, listingRecords, listingProfiles, invitations] = await Promise.all([
        ...
        base44.asServiceRole.entities.DirectoryListingAccess.list('-created_date', 1000),
