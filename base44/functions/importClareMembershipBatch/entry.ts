...
async function processBatch(base44:any,members:any[]){
 ...
 const newConsents:any[]=[];for(const m of members){const p=byKey.get(key(m.full_name,m.primary_email));let consentRows:any[]=[];if(m.consent_bundle){let statuses:any=m.consent_bundle.statuses||{};if(m.consent_bundle.pattern==='all_accepted')statuses=Object.fromEntries(Object.keys(CONSENT_DEFAULTS).map(k=>[k,'accepted']));if(m.consent_bundle.pattern==='photo_declined')statuses={...Object.fromEntries(Object.keys(CONSENT_DEFAULTS).map(k=>[k,'accepted'])),photography_video:'declined'};if(m.consent_bundle.pattern==='legacy_partial')statuses={gdpr_membership_communications:'accepted',membership_conditions:'accepted',health_declaration:'not_recorded',club_rules_code_of_conduct:'not_recorded',photography_video:'not_recorded',declaration_liability_waiver:'not_recorded'};consentRows=Object.entries(statuses).map(([consent_type,status])=>({consent_type,status,recorded_at:m.consent_bundle.recorded_at,source_row:m.consent_bundle.source_row,consent_version:m.consent_bundle.consent_version||'2026-27'}));}else consentRows=m.consents||[];for(const c of consentRows){const ck=`${p.id}|${c.consent_type}|${c.source_row}|${c.consent_version}`;if(ckeys.has(ck))continue;ckeys.add(ck);const d=CONSENT_DEFAULTS[c.consent_type]||{};newConsents.push(compact({tenant_id:TENANT,club_id:CLUB,person_id:p.id,consent_type:c.consent_type,status:c.status,response_text:c.response_text||d.response_text,consent_version:c.consent_version,wording_hash:c.wording_hash||d.wording_hash,recorded_at:c.recorded_at,source_system:SOURCE,source_row:c.source_row}));}}await bulkCreate(base44,'ConsentRecord',newConsents);
