...
Deno.serve(async(req)=>{try{
 ...
 if(liveScoresOnly){
  ...
  const now=Date.now();const scoreFields=(matches||[]).map((m:any)=>{const lockActive=!!(m.scoring_lock_owner&&m.scoring_lock_expires_at&&Date.parse(m.scoring_lock_expires_at)>now);return{id:m.id,status:m.status,team_a_score:m.team_a_score,team_b_score:m.team_b_score,winner_side:m.winner_side,serving_side_at_horn:m.serving_side_at_horn,result_method:m.result_method,revision:Number(m.revision||0),correction_count:Number(m.correction_count||0),completed_at:m.completed_at,last_corrected_at:m.last_corrected_at,last_corrected_by_user_id:m.last_corrected_by_user_id,last_correction_reason:m.last_correction_reason,original_result_json:m.original_result_json,command_id:m.command_id,scoring_lock_active:lockActive,scoring_lock_kind:lockActive&&(String(m.scoring_lock_owner||'').startsWith('host:'))?'host':lockActive?'player':'none',scoring_lock_expires_at:lockActive?m.scoring_lock_expires_at:null};});
