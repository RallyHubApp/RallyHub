...
Deno.serve(async(req)=>{try{
 ...
 if(!sessionId||!roundId||!commandId)return Response.json({error:'sessionId, roundId and commandId are required'},{status:400});
 ...
 let allowed=user.role==='admin';if(!allowed){const grants=await base44.asServiceRole.entities.KotcSessionAccess.filter({session_id:session.id,user_id:user.id,status:'active'});allowed=(grants||[]).some((a:any)=>validAccess(a,session.tenant_id,session.id));}if(!allowed)return Response.json({error:'Primary session host access required'},{status:403});
 ...
 // Session revision is advisory for START. A pair-lock, contact refresh or other harmless
 ...
 if(changedCourts.size){const matches=await base44.asServiceRole.entities.KotcMatch.filter({round_id:round.id,session_id:session.id});for(const m of matches){const rank=Number(m.ladder_court_rank);if(!changedCourts.has(rank))continue;const court=slots.map((s:any,i:number)=>({...s,participant_id:nextIds[i]})).filter((s:any)=>Number(s.ladder_court_rank)===rank);await base44.asServiceRole.entities.KotcMatch.update(m.id,{team_a_participant_ids:court.filter((s:any)=>s.team_side==='A').sort((a:any,b:any)=>Number(a.slot_number)-Number(b.slot_number)).map((s:any)=>s.participant_id),team_b_participant_ids:court.filter((s:any)=>s.team_side==='B').sort((a:any,b:any)=>Number(a.slot_number)-Number(b.slot_number)).map((s:any)=>s.participant_id),revision:Number(m.revision||0)+1,command_id:commandId});}}
 const now=nowIso();
 const update:any={status:session.status==='ready'?'in_progress':session.status,revision:Number(session.revision||0)+1,last_command_id:commandId,current_round_number:round.round_number,current_round_id:round.id};if(!session.actual_first_round_start)update.actual_first_round_start=now;
 const [updatedRound,updatedSession]=await Promise.all([
   base44.asServiceRole.entities.KotcRound.update(round.id,{status:'started',confirmed_at:now,confirmed_by_user_id:user.id,started_at:now}),
