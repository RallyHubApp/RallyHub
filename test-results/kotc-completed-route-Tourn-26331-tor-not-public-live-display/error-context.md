...
> 28 |   await expect(page.getByText('830 Session').first()).toBeVisible();
  ...
  47 |   await expect(page.getByTestId('kotc-email-subject')).toHaveValue('830 Session — your results');
  48 |   await expect(page.getByTestId('kotc-email-body')).toContainText('Hi [First name]');
  49 |   await expect(page.getByTestId('kotc-email-body')).toContainText('View your King of the Court results');
  50 |   await expect(page.getByTestId('kotc-email-body')).toContainText('Brian Moore');
  51 |   await expect(page.getByTestId('kotc-email-body')).toContainText('Clare Pickleball');
  52 |   await expect(page.getByTestId('kotc-email-body')).toContainText('Results powered by RallyHub');
  53 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('16 players · 1 guest/unlinked excluded');
  54 |   await expect(page.getByTestId('kotc-email-test')).toContainText('Send test email to Brian Moore');
  55 |   await expect(page.getByTestId('kotc-email-test')).toBeEnabled();
  56 |   await expect(page.getByTestId('kotc-email-send-all')).toHaveCount(0);
  57 |   await page.getByTestId('kotc-email-subject').fill('Thursday KOTC — results');
  58 |   await page.getByTestId('kotc-email-body').fill('Hi [First name],\n\nThanks for a great night. Your results are here.\n\nRegards,\nBrian Moore\nSession Host\nClare Pickleball');
  59 |   await page.getByTestId('kotc-email-test').click();
  60 |   await expect(page.getByTestId('kotc-email-status')).toContainText('Test email sent to brian.moore007@gmail.com');
  61 |   await expect(page.getByTestId('kotc-email-send-all')).toBeEnabled();
  62 |   expect(emailPayloads[0].subject).toBe('Thursday KOTC — results');
  63 |   expect(emailPayloads[0].messageBody).toContain('Hi [First name]');
  64 |   await page.getByTestId('kotc-email-body').fill('Hi [First name],\n\nUpdated wording.\n\nRegards,\nBrian Moore\nSession Host\nClare Pickleball');
  65 |   await expect(page.getByTestId('kotc-email-send-all')).toHaveCount(0);
  66 |   await expect(page.getByTestId('kotc-email-test')).toContainText('Send test email to Brian Moore');
  67 |   await expect(page.getByTestId('kotc-email-status')).toContainText('Email changed');
  68 |   expect(functionCalls.filter(x=>x==='kotcResultsShare')).toHaveLength(3);
