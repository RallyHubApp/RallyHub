...
Locator: getByTestId('kotc-email-players')
Expected substring: "Retry preview"
Received string:    "Preview failed — retry"
Timeout: 3000ms

Call log:
  - Expect "toContainText" getByTestId('kotc-email-players') with timeout 3000ms
  - waiting for getByTestId('kotc-email-players')
    10 × locator resolved to <button aria-busy="false" data-dynamic-content="true" data-testid="kotc-email-players" data-source-location="src/components/kotc/KotcV2SessionView.jsx:201:3732" class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border border-input bg-transparent shadow-sm…>…</button>
       - unexpected value "Preview failed — retry"

```

```yaml
- button "Preview failed — retry":
  - img
  - text: Preview failed — retry
```
...
```ts
  ...
  15 |   const errors=[];const functionCalls=[];let previewAttempts=0;page.on('pageerror',e=>errors.push(e.message));
...
> 43 |   await expect(page.getByTestId('kotc-email-players')).toContainText('Retry preview');
     |                                                        ^ Error: expect(locator).toContainText(expected) failed
  ...
  47 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('830 Session — your results');
  48 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('Hi [First name]');
  49 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('View your King of the Court results');
  50 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('Brian Moore');
  51 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('Clare Pickleball');
  52 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('Results powered by RallyHub');
  53 |   await expect(page.getByTestId('kotc-email-preview')).toContainText('16 players · 1 guest/unlinked excluded');
  54 |   await expect(page.getByTestId('kotc-email-send-all')).toBeDisabled();
  55 |   await page.getByTestId('kotc-email-test').click();
  56 |   await expect(page.getByTestId('kotc-email-status')).toContainText('Test email sent to brian.moore007@gmail.com');
  57 |   await expect(page.getByTestId('kotc-email-send-all')).toBeEnabled();
  58 |   expect(functionCalls.filter(x=>x==='kotcResultsShare')).toHaveLength(3);
