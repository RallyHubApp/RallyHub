...
Locator: getByTestId('cc-root')
Expected: visible
Timeout: 3000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByTestId('cc-root') with timeout 3000ms
  - waiting for getByTestId('cc-root')
...
> 209 |   await expect(page.getByTestId('cc-root')).toBeVisible();
  ...
  263 | });
  264 | 
  265 | test('Club Challenge public Hall Display robot: now/resting/next + disconnect truth',async({page},testInfo)=>{
  266 |   const participants=Array.from({length:20},(_,i)=>({id:`hp${i+1}`,display_name:`Player ${i+1}`}));
  267 |   const names=ids=>ids.map(id=>participants.find(p=>p.id===id)?.display_name||'Player');
