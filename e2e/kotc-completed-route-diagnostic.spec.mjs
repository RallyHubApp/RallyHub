...
test('completed KOTC tournament opens host review/editor, not public live display',async({page})=>{
  ...
  page.on('request',r=>{if(r.url().includes('/api/apps/'))console.log('REQ',r.method(),r.url())});
  page.on('response',r=>{if(r.url().includes('/api/apps/'))console.log('RES',r.status(),r.url())});
  await page.goto('/e2e/kotcCompletedRouteHarness.html');
  await page.waitForTimeout(1200); console.log('BODY',await page.locator('body').innerText()); console.log('ERRORS',errors);
