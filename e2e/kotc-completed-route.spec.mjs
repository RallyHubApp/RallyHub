...
test('Tournament Control Centre → completed KOTC opens host review/editor, not public live display',async({page})=>{
  ...
  await expect(page.getByTestId('kotc-email-subject')).toHaveValue('830 Session — results & player link');
  await expect(page.getByTestId('kotc-email-body')).toContainText('Hi [First name]');
  await expect(page.getByTestId('kotc-email-body')).toContainText('same player link you used during the session');
  await expect(page.getByTestId('kotc-email-body')).toContainText('https://rallyhub.ie/kotc-score/player-token');
  await expect(page.getByTestId('kotc-email-body')).toContainText('simple infographic showing how to use the player link');
  await expect(page.getByTestId('kotc-email-body')).toContainText('https://base44.app/api/apps/test/files/player-guide.png');
